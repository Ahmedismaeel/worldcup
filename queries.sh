...
echo "$($PSQL "SELECT DISTINCT(t.name) AS team_name FROM games g JOIN teams t ON t.team_id=g.winner_id OR t.team_id=g.opponent_id  WHERE g.year='2014' AND g.round='Eighth-Final' ")"
